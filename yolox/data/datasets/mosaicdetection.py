...
class DiffusionMosaicDetection(Dataset):
    """Detection dataset wrapper that performs mixup for normal dataset."""
    ...
    def __getitem__(self, idx):
        if self.enable_mosaic:
            ...
            for i_mosaic, index in enumerate(ref_indices):
                img, _labels, _, _ = self._dataset.pull_item(index)
                h0, w0 = img.shape[:2]  # orig hw
                scale = min(1. * input_h / h0, 1. * input_w / w0)
                img = cv2.resize(
            ...
            mosaic_labels = []
            for i_mosaic, index in enumerate(track_indices):
                img, _labels, _, _ = self._dataset.pull_item(index)
                h0, w0 = img.shape[:2]  # orig hw
                scale = min(1. * input_h / h0, 1. * input_w / w0)
                img = cv2.resize(
            ...
            return ref_mix_img, ref_padded_labels,track_mix_img, track_padded_labels,img_info, np.array([idx])
            ...
            cur_idx_range=[idx>=self.video_info[k][0] and idx<=self.video_info[k][1]  for k in video_keys]
            cur_idx_video_key=video_keys.pop(cur_idx_range.index(True))
            ref_img, ref_label, img_info_r, id_r = self._dataset.pull_item(idx)
            track_idx=max(min(idx+random.randint(-self.track_range,self.track_range),self.video_info[cur_idx_video_key][1]),self.video_info[cur_idx_video_key][0])
            track_img, track_label, img_info_t, id_t = self._dataset.pull_item(track_idx)
            ref_img, ref_label,track_img, track_label = self.preproc(ref_img, ref_label, track_img, track_label,self.input_dim)
            return ref_img, ref_label, track_img, track_label,img_info_r,id_r
